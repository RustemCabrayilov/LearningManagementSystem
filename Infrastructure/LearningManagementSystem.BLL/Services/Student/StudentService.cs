...
public class StudentService(
    ...
    IGenericRepository<StudentMajor> _studentMajorRepository,
    ...
    IUnitOfWork _unitOfWork) : IStudentService
{
    ...
    public async Task<StudentResponse> GetAsync(Guid id)
    {
        ...
        foreach (var studentGroup in studentGroups)
        {
            var group = await _groupRepository.GetAsync(x =>
                !x.IsDeleted && x.Id == studentGroup.Group.Id && x.TermId == term.Id);
        ...
        var studentExamList = new List<Domain.Entities.StudentExam>();
        foreach (var studentExam in studentExams)
        {
            var exam = await _examRepository.GetAsync(x => x.Id == studentExam.ExamId&&!x.IsDeleted);
        ...
        {
            var retakeExam = await _retakeExamRepository.GetAsync(x => x.Id == studentRetakeExam.RetakeExamId&&!x.IsDeleted);
        ...
        {
            Groups = groupList,
            StudentExams = _mapper.Map<IList<StudentExamResponse>>(studentExams),
    ...
    public async Task<StudentGroupDto[]> AssignGroupsAsync(StudentGroupDto[] dtos)
    {
        ...
        {
            var studentGroup = await _studentGroupRepository.GetAsync(x =>x.StudentId==entity.StudentId&& x.GroupId == entity.GroupId && !x.IsDeleted);
            if (studentGroup is not null)
            {
